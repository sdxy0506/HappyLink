	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < map.length - 1; i++) {
			for (int j = 0; j < map[i].length - 1; j++) {
				if (map[i][j] == 0)
					canvas.drawRect(j * icon_size, i * icon_size, j * icon_size
							+ icon_size, i * icon_size + icon_size, paint);
				else
					canvas.drawBitmap(icons[map[i][j]], j * icon_size, i
							* icon_size, paint_bit);
			}
		}
		link(canvas);
		...
		for (Point position : selected) {
			...
			if (map[position.x][position.y] >= 1) {
				canvas.drawBitmap(icons[map[position.x][position.y]], null,
						new Rect(p.x - 8, p.y - 8, p.x + icon_size + 8, p.y
								+ icon_size + 8), null);
			}
		}
	}
